...
// ==========================================
// FixAI Mailer
// Supports SendGrid (Best for Render), Brevo, Resend, and SMTP
// ==========================================

let sendMailFn;

// ---------- Option 1: SendGrid HTTP API (Best for Render, no IP restrictions) ----------
if (process.env.SENDGRID_API_KEY) {

  console.log('📧 Email provider: SendGrid (HTTP API)');
  const senderEmail = process.env.MAIL_FROM_EMAIL || process.env.OWNER_EMAIL || 'mayank5406jangid@gmail.com';
  const senderName = process.env.MAIL_FROM_NAME || 'FixAI';

  sendMailFn = async function sendMailSendGrid(to, subject, html) {
    try {
      ...
      if (!res.ok) {
        ...
        console.error(`❌ SendGrid email FAILED to ${to}:`, errorText);
